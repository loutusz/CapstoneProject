...
func InitMessageProviderHTTPHandler(uq messageprovider.UsecaseQuery, uc messageprovider.UsecaseCommand, s *servers.GinServer) {
	...
	s.Gin.GET("/message-provider/user/connected/:id", handler.MessageProviderUsecaseQuery.GetConnectedUserOwned)
	...
	s.Gin.DELETE("/message-provider/id/:id", handler.MessageProviderUsecaseCommand.DeleteMessageProvider)
}
